					  ...
					  $.ajax({
						 ...
						 success:function(json){
			               if(json.state){
			            	    ...
			            	    var xfhfyk = $(".xfhfyk").text();
			                    
			                   
			                  	if((parseFloat(needPay)+parseFloat(xfhfyk))>parseFloat(availablePredeposit)){
		            ...
		            <li>
		                <a href="">
		                    ...
		                    <div class="navs4_right">
		                      ...
		                      <span > 预付费：¥ <lable class="yff">${goods.bookDownPayment/100}</lable></span>
		                       </p>
...
/*	  $(".pay_delete").next().on('input',function(e){  
       var number=$(".pay_delete").next().val();
       
       if(number<1){
          $(".pay_delete").next().val(1);
           number=$(".pay_delete").next().val();
       }
       
       var price = $(".price").val();                             //产品单价
       var tprice=$("#red_money").find('i').text();    //红包价格
       var yff =$(".yff").text()  == ''?0:parseInt($(".yff").text());  //预付定金单价
       ...
       if(tprice>yff){
		  ...
		  //商品总额
       	  $(".sub-total").find('i').text(parseFloat(price)*parseInt($(".pay_delete").next().val()));               //总金额多少钱
	      $(".total").text(needtotalpay(yff,number,tprice));//总价
	      $(".zxyf").text("￥"+needtotalpay(yff,number,tprice));//在线预付
	      var xfhfyk=(price-yff)*number;
	      $(".xfhfyk").text(xfhfyk);
	      $(".needpay").text(needtotalpay(yff,number,tprice));//应付
	      $("input[name='map[needpay]']").val(needtotalpay(yff,number,tprice));
...
function queryMoney(sum){
		 var price = $(".price").val();
	 	var tprice=$("#red_money").find('i').text();
	    $(".sub-total").find('i').text(parseFloat(parseFloat(price)*parseInt(sum)).toFixed(2));
	    //$(".total").text(parseFloat(price)*parseInt(sum)-parseFloat(tprice));
	    //$(".needpay").text(parseFloat(price)*parseInt(sum)-parseFloat(tprice));
	    $("input[name='map[needpay]']").val(parseFloat(parseFloat(price)*parseInt(sum)-parseFloat(tprice)).toFixed(2));
	     
	    var zxyfMoney =$(".yff").text()  == ''?0:parseFloat($(".yff").text()) *sum;
	    zxyfMoney =  (zxyfMoney)  - tprice;
	    
	    var vipPrice= $(".vipPrice").text()  == ''?0:parseFloat($(".vipPrice").text());
	    var yff=$(".yff").text()  == ''?0:parseFloat($(".yff").text())
	    var xfhfyk=(vipPrice-yff)*sum;
	    
	    $(".xfhfyk").text(parseFloat(xfhfyk).toFixed(2));
	    
	    $(".zxyf").text("￥"+parseFloat(zxyfMoney).toFixed(2));
	    $(".needpay").text(parseFloat(zxyfMoney).toFixed(2));
	    $(".total").text(parseFloat(zxyfMoney).toFixed(2));
}
</script>
...
<form>	    
		    ...
		    <section class="navs25">
		        ...
		        <div class="navs20_d1"><span class="navs20_d1_s1">在线预付</span><span class="navs20_d1_s3 zxyf">¥${goods.bookDownPayment/100!''}</span> </div>
		        <div class="navs20_d1"><span class="navs20_d1_s1">消费后付余款</span><span class="navs20_d1_s3">¥<lable class="xfhfyk">
		         <#if LOGIN_SIGN  ?? >
			                        <#if LOGIN_SIGN.vipRankId==0 >
			                        	<lable class="vipPrice" > ${(goods.goodsVip0Price/100!)-(goods.bookDownPayment/100)}</lable>
			                        <#elseif  LOGIN_SIGN.vipRankId==1 >
			                        	<lable class="vipPrice" > ${(goods.goodsVip1Price/100!)-(goods.bookDownPayment/100)}</lable>
			                        <#elseif  LOGIN_SIGN.vipRankId==2 >
			                       		<lable class="vipPrice" > ${(goods.goodsVip2Price/100!)-(goods.bookDownPayment/100)}</lable>
			                        <#elseif  LOGIN_SIGN.vipRankId==3 >
			                       		<lable class="vipPrice" > ${(goods.goodsVip3Price/100!)-(goods.bookDownPayment/100)}</lable>
			                        <#elseif  LOGIN_SIGN.vipRankId==4 >
			                        	<lable class="vipPrice" > ${(goods.goodsVip4Price/100!)-(goods.bookDownPayment/100)}</lable>
			                        </#if>
		                        <#else>
		                        	 	<lable class="vipPrice" > ${(goods.goodsVip0Price/100!)-(goods.bookDownPayment/100)}</lable>
		                        </#if>
		    ...
		    <p class="navs8_p2">预付费：<i>￥</i><span class="total">${goods.bookDownPayment/100!'0'}</span><a href="javascript:;" class="pay_lj">提交订单</a> </p>
</form>		    
	  ...
	  $(".lb_s").click(function(){
		var val=$('input:radio[name="hb_c"]:checked').val();
	                ...
	                $(".pay_bt3_yes").attr("disabled", false);
						  ...
						  $(".pay_bt3_yes").click(function(){
						  		...
						  		if(type != null && type != '' && type != 'undefined'){
						  			...
						  			var money = type.parents("p").find(".red_money").text();//红包金额
						  			money = convert(money);
						  			var yff = $(".yff").text()  == ''?0:parseInt($(".yff").text());//定金单价金额
						  			yff = convert(yff);
						  			...
						  			$("#red_money").find('i').text(parseFloat(money).toFixed(2));//优惠金额
						  			  
						  			var zxyfMoney =yff *num.val();
						  			  if(zxyfMoney - money >=0){
						  			  	//$('.sub-total').find('i').text(parseFloat(totalMoney-money));
						
							    		 zxyfMoney =   zxyfMoney - money;
							        	 $(".zxyf").text(parseFloat(zxyfMoney).toFixed(2));
							        	 $('.needpay').text(parseFloat(zxyfMoney).toFixed(2));
							          	$('.total').text(parseFloat(zxyfMoney).toFixed(2));
						  			  	
						  			  }else{
						  			  	$('.sub-total').find('i').text(0);
						  			  	//needpay 需付款
						  			  	$('.needpay').text(0);
						  			  	$('.total').text(0);
						  			  }
		       ...
		       <form>	 
		         ...
		         <input type="hidden" name="map[needpay]" value="${goods.bookDownPayment/100!'0'}">    <!-- 应付金额 -->
		        ...
		        <div class="pay_bt1_one pay_bt1_two">
		           ...
		           <p class="pay_bt1_p2">VIP账户<span>账户余额:<lable class="zhyebalance">0</lable>元</span></p>
		            ...
		            <p class="pay_bt1_p5"><span>需付款</span><i>元</i><i class="needpay">${goods.bookDownPayment/100!'0'}</i></p>
