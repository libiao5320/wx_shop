...
function ajaxload(pageNo){
	$.ajax({
		url: '/store/evaluatelist.ajax' ,
		type: 'POST',
		data: {'map[pageNo]':pageNo,'map[storeId]':storeId,'map[isShow]':'Y'} ,
		dataType: 'json',
...
function evaluatelist(data){
	...
	if(data != 'undefined' && data.length > 0){
		for(var i=0;i < data.length ;i ++){
			...
			if(data[i].member != '' && data[i].member != 'undefined' && data[i].member.memberMobile != '' && data[i].member.memberMobile != 'undefined')
			...
			else if(data[i].member != '' && data[i].member != 'undefined' && ata[i].member.memberName != '' && ata[i].member.memberName != 'undefined')
...
<article>
    <section class="navs13">
        ...
        <ul class="navs13_ul" id="evaluate">
            <#if list?? && (list?size >0)>
        	<#list list as eve>
            <li class="navs13_li">${eve_index }
                <div class="navs13_left">
                	<#if eve.member??>
                    <img src="${eve.member.memberAvatar !''}" title="" alt="">
                    <#else>
                ...
                <div class="navs13_right">
                    <p class="navs13_right_p1">
                    	<#if eve?? && eve.member ?? && eve.member.memberMobile??>
                    	${eve.member.memberMobile?substring(0,2)}****${eve.member.memberMobile?substring(7,10)}
                    	<#elseif eve?? && eve.member ??>
                    	${eve.member.memberName !''}
                    	<#else>
                    	匿名用户
                    	</#if>
                    	<span>${eve.strCreateTime !''}</span>
                    </p>
                    <p class="navs13_right_p2">
                    	<#if eve.grade?? && eve.grade == 0>
                    	<i></i><i></i><i></i><i></i><i></i>
                    	<#elseif eve.grade?? && eve.grade == 1>
                    	<i class="navs13_i"></i><i></i><i></i><i></i><i></i>
                    	<#elseif eve.grade?? && eve.grade == 2>
                    	<i class="navs13_i"></i><i class="navs13_i"></i><i></i><i></i><i></i>
                    	<#elseif eve.grade?? && eve.grade == 4>
                    	<i class="navs13_i"></i><i class="navs13_i"></i><i class="navs13_i"></i><i class="navs13_i"></i><i class=""></i>
                    	<#elseif eve.grade?? && eve.grade == 5>
                ...
                <p class="navs13_p1">${eve.detail !''}</p>
                <#if eve?? && eve?size gt 0>
                <ul>
                	<#if eve.imageList?? && eve.imageList?size gt 0>
                	<#list eve.imageList as image>
