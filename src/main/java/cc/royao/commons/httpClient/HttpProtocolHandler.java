	...
	public HttpResponse execute(HttpRequest request) throws HttpException, IOException {
		...
		if (request.getMethod().equals(HttpRequest.METHOD_GET)) {
			...
			for (int i = 0; i < request.getParameters().length; i++) {
				((PostMethod) method).addParameter(request.getParameters()[i].getName(), request.getParameters()[i].getValue());
			}
		}

		// 设置Http Header中的User-Agent属性
		method.addRequestHeader("User-Agent", "http://www.zhiqiyun.com/common/4.0");
